...
namespace LLDB {
	...
	void MySQLSession::query(const std::string& sql, 
		...
		for (auto& s : sqls) {
			...
			auto errc = mysql_real_query(db, sql.c_str(), sql.size());
			if (errc != OK) {
				throw buildException(db);
			}
			auto res = mysql_use_result(db);
			if (res) {
	...
	void MySQLSession::setCurrentDB(const std::string& dbname) {
		if (mysql_select_db(db, dbname.c_str()) != OK) {
			throw buildException(db);
		}
	}
